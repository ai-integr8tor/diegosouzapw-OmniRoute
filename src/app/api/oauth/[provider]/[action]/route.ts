...
// Devin Desktop / CLI retired callback state (separate from Codex)
if (!globalThis.__devinCallbackState) {
  globalThis.__devinCallbackState = null;
}
...
/**
 * Start a PKCE callback server for Codex or Devin compatibility flows.
 * Codex uses fixed port 1455; Devin uses a random free port (port 0).
...
async function handleStartCallbackServer(
  ...
  const isDevin = provider === "devin-desktop" || provider === "devin-cli";
  const stateKey = isDevin ? "__devinCallbackState" : "__codexCallbackState";
  ...
  try {
    // Codex: fixed port 1455. Devin: OS-assigned random port (0).
    const serverPort = isDevin ? 0 : 1455;
...
export async function POST(
  ...
  try {
    ...
    if (action === "poll-callback") {
      ...
      // Devin Desktop and CLI share callback state; Codex uses its own slot.
      const stateKey = provider === "codex" ? "__codexCallbackState" : "__devinCallbackState";
