...
// waiting for the next event. User asked: why didn't the combo fall over to the
// next target instead of returning a half-finished stream?
//
// Fix: the streaming validator now passes through only when it actually saw a
// recognised SSE terminator ([DONE], `message_stop`, `finish_reason`, terminal
// `usage`) OR structured SSE activity (parsed `data:` / `event:` frames).
// Raw bytes that never produced a parseable event now correctly mark invalid.
