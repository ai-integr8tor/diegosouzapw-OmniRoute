...
describe("Claude Web live request alignment", () => {
  ...
  it("matches the advertised browser version to the TLS profile and uses the new-chat referer", async () => {
    clearBrowserFlags();
    ...
    assert.match(capturedHeaders?.["User-Agent"] ?? "", /Chrome\/146\./);
    assert.match(capturedHeaders?.["Sec-Ch-Ua"] ?? "", /v="146"/);
